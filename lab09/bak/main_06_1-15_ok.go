...
func main() {
	...
	// f, err := os.Open("/Users/brisk/vscode/contest/lab09/tests/03")
	// if err != nil {
	// 	log.Fatal(err)
	// }
	// defer f.Close()
	...
	for i := 0; i < n; i++ {
		...
		// word = "decide"
		l := len(word)
		tree.Insert(StringVal{word, []string{word}})
		for ml := l; ml > 0; ml-- {
			for i := 0; i <= l-ml; i++ {
				key := word[i : ml+i]
				// fmt.Println(key)
				tree.Insert(StringVal{key, []string{word}})
			}
		}
		// tEnds
		tEnds.Insert(StringVal{word, []string{word}})
		for ml := l - 1; ml > 0; ml-- { // l-0 ???
			key := word[ml:l]
			// fmt.Println(ml, key, word)
			// tn := len(key) - 1
			tEnds.Insert(StringVal{key, []string{word}})
		}
		// break
	}
	// fmt.Println("\n\n\n")
	...
	for i := 0; i < q; i++ {
		...
		node := tEnds.root.get(StringVal{req, nil}) // l-1
		if node != nil {
		...
		for ml := l; ml > 0; ml-- { // l-0 ???
			key := req[ml:l]

			node := tEnds.root.get(StringVal{key, nil}) // l-1
			if node != nil {
				word := node.Value.(StringVal).word
				br = false
				for _, v := range word {
		...
		if foundWord != "" {
			// fmt.Println(req, foundWord, "~~~", foundSuff)
		...
		node = tree.root.get(StringVal{req, nil}) // l-1
		if node != nil {
			word := node.Value.(StringVal).word
			br = false
			for _, v := range word {
		...
		for ml := l - 1; ml > 0; ml-- {
			for i := 0; i <= l-ml; i++ {
				...
				node := tree.root.get(StringVal{key, nil}) // l-1
				if node != nil {
					word := node.Value.(StringVal).word
					br = false
					for _, v := range word {
